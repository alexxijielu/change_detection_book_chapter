...
'''Convert input images to RGB format in separate folders required by MRCNN, only for images matching a string
...
def preprocess_images_with_string(inputdirectory, outputdirectory, outputfile, string, verbose=False):
    if inputdirectory[-1] != "/":
        inputdirectory = inputdirectory + "/"
    if outputdirectory[-1] != "/":
        outputdirectory = outputdirectory + "/"
...
'''Convert input images to RGB format in separate folders required by MRCNN, without stretching of input
Arguments:
inputdirectory: Input directory containing images.
outputdirectory: Output directory to put new files in.
outputfile: Path to save comma-delimited file that will tell the neural network the image paths.'''
...
def preprocess_images_without_rescale(inputdirectory, outputdirectory, outputfile, string='', verbose=False):
    if inputdirectory[-1] != "/":
        inputdirectory = inputdirectory + "/"
    if outputdirectory[-1] != "/":
        outputdirectory = outputdirectory + "/"
